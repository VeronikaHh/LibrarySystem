...
def test_get_all_employees(employees_service: EmployeeService, employees: list[Employee]) -> None:
...
def test_get_employee_by_id_not_found(employees_service: EmployeeService, employees: list[Employee]) -> None:
    with pytest.raises(EmployeeNotFoundException):
        employees_service.get_employee_by_id(uuid.uuid4())


def test_create_employee(employees_service: EmployeeService, create_employee_request: EmployeeCreate) -> None:
    created_employee = employees_service.create_employee(employee=create_employee_request)
...
def test_delete_employee(employees_service: EmployeeService, employee_without_orders: Employee) -> None:
    employees_service.delete_employee(employee_id=employee_without_orders.employee_id)
    with pytest.raises(EmployeeNotFoundException):
        employees_service.get_employee_by_id(employee_without_orders.employee_id)
